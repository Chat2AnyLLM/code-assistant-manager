...
class MCPMSchemaMigrator:
    ...
    def __init__(self, mcpm_path: str, code_assistant_manager_path: str, ignore_keys: Optional[List[str]] = None):
        ...
        self.code_assistant_manager_servers_dir = self.code_assistant_manager_path / "code_assistant_manager" / "mcp" / "registry" / "servers"
    ...
    def _collect_detailed_differences(self, server_name: str, existing: Dict[str, Any], migrated: Dict[str, Any]) -> None:
        ...
        key_fields = ['installations', 'description', 'author', 'license', 'categories', 'tags', 'homepage', 'repository']
        ...
        for field in key_fields:
            ...
            if existing_val != migrated_val:
                ...
                change_type = self._determine_change_type(field, existing_val, migrated_val)
            ...
            if added:
                for method in added:
                    self.detailed_differences.append({
                        ...
                        'upstream': self._format_installation_method(migrated_installs[method]),  # upstream has it
            ...
            if removed:
                for method in removed:
                    self.detailed_differences.append({
                        ...
                        'local': self._format_installation_method(existing_installs[method]),  # local has it
    ...
    def _format_value_for_display(self, value: Any) -> str:
        ...
        if value is None:
            ...
            return ", ".join(str(item) for item in value[:3]) + (f" (+{len(value)-3} more)" if len(value) > 3 else "")
    ...
    def _determine_change_type(self, field: str, existing_val: Any, migrated_val: Any) -> str:
        ...
        elif field == 'installations' and isinstance(existing_val, dict) and isinstance(migrated_val, dict):
    ...
    def _print_json_diff(self, existing: Dict[str, Any], migrated: Dict[str, Any]) -> None:
        ...
        diff = difflib.unified_diff(existing_lines, migrated_lines, fromfile='existing', tofile='migrated', lineterm='')
        print('\n'.join(diff))
    ...
    def migrate_all_servers(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        if not self.mcpm_servers_dir.exists():
            raise FileNotFoundError(f"mcpm.sh servers directory not found: {self.mcpm_servers_dir}")

        if not self.code_assistant_manager_servers_dir.exists():
            if not dry_run:
                self.code_assistant_manager_servers_dir.mkdir(parents=True, exist_ok=True)
            print(f"Created code-assistant-manager servers directory: {self.code_assistant_manager_servers_dir}")
        ...
        target_files = list(self.code_assistant_manager_servers_dir.glob("*.json")) if self.code_assistant_manager_servers_dir.exists() else []
        ...
        for server_file in sorted(source_files):
            try:
                ...
                code_assistant_manager_file = self.code_assistant_manager_servers_dir / f"{filename}.json"
                ...
                if code_assistant_manager_file.exists():
                    try:
                        with open(code_assistant_manager_file, 'r', encoding='utf-8') as f:
                            existing_data = json.load(f)
                    except Exception as e:
                        print(f"Warning: couldn't read existing file {code_assistant_manager_file}: {e}")
                        existing_data = None
                    ...
                    if existing_data is not None:
                        ...
                        existing_json = json.dumps(existing_cmp, sort_keys=True, ensure_ascii=False)
                        migrated_json = json.dumps(migrated_cmp, sort_keys=True, ensure_ascii=False)
                        ...
                        if existing_json == migrated_json:
                            ...
                            self._collect_detailed_differences(server_name, existing_cmp, migrated_cmp)

                            if dry_run:
                                print(f"Would overwrite {server_name}: differences detected between source and target")
                                ...
                                if getattr(self, 'force', False):
                                    ...
                                    overwrite = typer.confirm(f"Overwrite target schema for '{server_name}'?", default=False)
                                    if not overwrite:
                                        print(f"Skipping {server_name}: user chose not to overwrite")
                                        skipped_count += 1
                                        continue
        ...
        return {
            ...
            "different": different_count + len(target_only_files),  # Include target-only files in different count
    ...
    def backup_existing_schemas(self) -> Path:
        ...
        """
        backup_dir = self.code_assistant_manager_path / "backup" / "servers_pre_migration"
...
app = typer.Typer(help="Migrate MCP server schemas from mcpm.sh to code-assistant-manager")


@app.command()
def main(
    mcpm_path: str = typer.Option(..., help="Path to mcpm.sh repository"),
    code_assistant_manager_path: str = typer.Option('.', help="Path to code-assistant-manager repository (default: current directory)"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show what would be migrated without making changes"),
    backup: bool = typer.Option(False, help="Create backup of existing schemas before migration"),
    force: bool = typer.Option(False, "--force", help="Automatically overwrite differing target schemas without prompting"),
    ignore_keys: Optional[List[str]] = typer.Option(None, help="Comma-separated list of keys to ignore when comparing schemas (e.g. updated_at,last_modified)")
):
    ...
    if ignore_keys:
        if isinstance(ignore_keys, str):
            parsed_ignore_keys = [k.strip() for k in ignore_keys.split(',') if k.strip()]
        elif isinstance(ignore_keys, list):
            parsed_ignore_keys = ignore_keys

    # Create migrator with provided ignore_keys
    migrator = MCPMSchemaMigrator(mcpm_path, code_assistant_manager_path, ignore_keys=parsed_ignore_keys)
    ...
    categories_data.append([
        ...
        str(missing_count),
        f"All {results['total_found']} MCP servers from mcpm.sh have local counterparts" if missing_count == 0 else "Some files need migration"
    ])

    # Category 2: Destination files not in source (locally added)
    local_icon = "➕" if results['target_only'] > 0 else "✅"
    local_details = f"**{', '.join(results['target_only_files'])}** - Microsoft Learn MCP server" if results['target_only'] > 0 else "No locally added files"
    categories_data.append([
    ...
    diff_details = f"{results['different'] - results['target_only']} modified, {results['target_only']} local-only" if results['different'] > results['target_only'] else f"{results['different']} total differences"
    categories_data.append([
    ...
    typer.echo(tabulate(categories_data, headers=["Category", "Count", "Details"], tablefmt="grid", maxcolwidths=[40, 8, 35]))
    typer.echo()

    # Registry synchronization status table
    typer.echo("📈 REGISTRY SYNCHRONIZATION STATUS:")
    identical_count = results['total_found'] - results['migrated'] - results['failed'] - (results['different'] - results['target_only'])
    sync_rate = (identical_count / results['total_found'] * 100) if results['total_found'] > 0 else 100

    status_data = [
        ["📊 Total upstream servers", str(results['total_found']), ""],
        ["📊 Total local servers", str(results['total_found'] + results['target_only']), ""],
        ["🎯 Synchronization rate", f"{sync_rate:.1f}%", f"({identical_count}/{results['total_found']} servers identical)"],
        ["✨ Local enhancements", str(results['target_only'] + (results['different'] - results['target_only'])), f"{results['target_only']} new, {results['different'] - results['target_only']} modified"]
    ]

    typer.echo(tabulate(status_data, headers=["Metric", "Value", "Details"], tablefmt="grid", maxcolwidths=[25, 15, 40]))
    typer.echo()
    ...
    if results['different'] > 0 and migrator.detailed_differences:
        ...
        for server_name, diffs in server_groups.items():
            ...
            typer.echo(tabulate(server_diff_data, headers=["Aspect", "Upstream (mcpm.sh)", "Local Registry", "Change Type"],
                              tablefmt="grid", maxcolwidths=[25, 25, 35, 15]))
    ...
    typer.echo(tabulate(legend_data, headers=["Symbol", "Meaning"], tablefmt="simple", colalign=("center", "left")))
    typer.echo("\n" + "="*90)
