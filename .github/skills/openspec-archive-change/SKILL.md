...
5. **Validate before archiving**
   ...
   Run `openspec validate --all --strict --no-interactive` with the same selected-root flags. Require a zero exit status. If validation fails, report the errors and stop before creating or moving any archive directory.
